...
import {
  ...
  getProjectServiceTrackingFormatPrompt,
  getFinanceTransactionsFormatPrompt,
  getBusinessReportFormatPrompt,
...
import {
  buildQAContext,
} from '@/lib/telegram/qa';
import {
  resolveCustomersBatch,
} from '@/lib/telegram/customers';
import type {
  TelegramSender,
} from '@/generated/prisma/client';
...
function parseFinanceTransactionText(text: string, fallbackDate: Date): FinanceRecord | null {
  ...
  const amountText = convertBurmeseDigits(value('amount mmk', 'amount', 'value')).replace(/[,\s]/g, '');
  ...
  const date = parseExcelDate(convertBurmeseDigits(value('date', 'record date'))) || fallbackDate;
  return {
    date,
    ...
    dueDate: parseExcelDate(convertBurmeseDigits(value('due date', 'due'))) || null,
